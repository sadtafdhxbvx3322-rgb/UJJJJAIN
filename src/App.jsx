...
const DEITY_FRAG = `
  ...
  void main() {
    ...
    float vein = fbm(vPosition * 5.0 + vec3(0.0, uTime * 0.08, 0.0), 4);
    vein = smoothstep(0.15, 0.55, vein);

    float core = smoothstep(-0.15, 0.35, vDisplace);
    vec3 baseColor = mix(uColorDeep, uColorGold, core);
    baseColor = mix(baseColor, uColorHot, vein * 0.5);

    float pulse = 0.5 + 0.5 * sin(uTime * 0.9);
    vec3 glow = uColorHot * fresnel * (1.4 + pulse * 0.6) * uIntensity;

    vec3 finalColor = baseColor + glow;
    finalColor += uColorGold * fresnel * 0.6;
...
const AURA_VERT = `
  ...
  void main(){
    vec3 n = normalize(normal);
    float breathing = sin(uTime * 0.6) * 0.08 + 1.0;
    vec3 pos = position * breathing;
    pos += n * fbm(position * 1.2 + uTime * 0.05, 3) * 0.15;
    vNormal = normalize(normalMatrix * n);
...
const AURA_FRAG = `
  ...
  void main(){
    vec3 viewDir = normalize(uCameraPos - vWorldPos);
    float fresnel = pow(1.0 - clamp(dot(viewDir, vNormal), 0.0, 1.0), 1.8);
    float flicker = 0.85 + 0.15 * sin(uTime * 2.3 + vWorldPos.x * 3.0);
    gl_FragColor = vec4(uColor * fresnel * flicker * 1.6, fresnel * 0.85);
  }
`;
...
const PARTICLE_VERT = `
  ...
  void main(){
    ...
    p.y += mod(t * (0.4 + riseSpeed), 6.0) - 1.0;

    float swirl = uTime * 0.25 + aPhase * 6.2831;
    float radius = length(aOrigin.xz) + sin(t * 0.5) * 0.15;
    float angle = atan(aOrigin.z, aOrigin.x) + swirl * (0.3 / max(radius, 0.4));
    p.x = cos(angle) * radius;
    p.z = sin(angle) * radius;

    p += vec3(
      fbm(aOrigin * 2.0 + uTime * 0.1, 2),
      fbm(aOrigin * 2.0 + uTime * 0.1 + 50.0, 2),
      fbm(aOrigin * 2.0 + uTime * 0.1 + 100.0, 2)
    ) * 0.25;

    vec3 toMouse = p - uMouse;
    float d = length(toMouse.xy);
    float repel = smoothstep(1.2, 0.0, d) * 0.6;
    p.xy += normalize(toMouse.xy + 0.0001) * repel;

    float cycle = fract(t * 0.12);
    vFade = smoothstep(0.0, 0.15, cycle) * smoothstep(1.0, 0.7, cycle);
...
const PARTICLE_FRAG = `
  ...
  void main(){
    ...
    vec3 color = uColorEmber;
    if (vType > 0.5 && vType < 1.5) color = uColorGold;
    if (vType >= 1.5) color = uColorAsh;
...
const BRIGHT_PASS_FRAG = `
  ...
  void main(){
    ...
    float contribution = smoothstep(uThreshold, uThreshold + 0.3, luma);
...
const BLUR_FRAG = `
  ...
  void main(){
    ...
    vec4 sum = vec4(0.0);
    sum += texture2D(uTex, vUv - texel * 4.0) * 0.0162162162;
    sum += texture2D(uTex, vUv - texel * 3.0) * 0.0540540541;
    sum += texture2D(uTex, vUv - texel * 2.0) * 0.1216216216;
    sum += texture2D(uTex, vUv - texel * 1.0) * 0.1945945946;
    sum += texture2D(uTex, vUv)               * 0.2270270270;
    sum += texture2D(uTex, vUv + texel * 1.0) * 0.1945945946;
    sum += texture2D(uTex, vUv + texel * 2.0) * 0.1216216216;
    sum += texture2D(uTex, vUv + texel * 3.0) * 0.0540540541;
    sum += texture2D(uTex, vUv + texel * 4.0) * 0.0162162162;
...
const COMPOSITE_FRAG = `
  ...
  void main(){
    ...
    float aberr = uAberration * length(centered);
    vec2 dir = normalize(centered + 0.0001);
    float r = texture2D(uScene, uv - dir * aberr * 0.004).r;
    float g = texture2D(uScene, uv).g;
    float b = texture2D(uScene, uv + dir * aberr * 0.004).b;
    ...
    float grain = (rand(uv + fract(uTime)) - 0.5) * uGrainStrength;
    color += grain;
...
const GROUND_FRAG = `
  ...
  void main(){
    ...
    float rings = sin(d * 18.0 - uTime * 1.4) * 0.5 + 0.5;
...
function buildParticleField(count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    origins[i * 3 + 0] = Math.cos(angle) * radius;
    origins[i * 3 + 1] = height;
    ...
    types[i] = typeRoll < 0.5 ? 0 : typeRoll < 0.8 ? 1 : 2;
    sizes[i] = types[i] === 1 ? 2.5 + Math.random() * 2.5 : 4 + Math.random() * 5;
  }
...
class CinematicScene {
  constructor(canvas) {
    ...
    this.renderer.setClearColor(0x030201, 1);
    this.renderer.outputColorSpace = THREE.SRGBColorSpace;

    this.scene = new THREE.Scene();
    this.scene.fog = new THREE.FogExp2(0x050403, 0.055);
  ...
  _buildLights() {
    ...
    this._rimBasePos = rim.position.clone();
  }
  ...
  _buildDeity() {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: DEITY_FRAG,
      uniforms: {
        uTime: { value: 0 },
        uIntensity: { value: 1.0 },
        uColorDeep: { value: new THREE.Color(0x0c0704) },
        uColorGold: { value: new THREE.Color(0xd4af6a) },
        uColorHot: { value: new THREE.Color(0xffb84d) },
        uCameraPos: { value: new THREE.Vector3() },
      },
    });
    this.deity = new THREE.Mesh(geometry, material);
    this.deity.position.y = 0.1;
    this.scene.add(this.deity);
  }

  _buildAura() {
    const geometry = new THREE.IcosahedronGeometry(1.62, 4);
    const material = new THREE.ShaderMaterial({
    ...
    this.aura.position.copy(this.deity.position);
  ...
  _buildGround() {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        uTime: { value: 0 },
        uColor: { value: new THREE.Color(0xd4af6a) },
      },
      transparent: true,
      blending: THREE.AdditiveBlending,
      depthWrite: false,
  ...
  _buildParticles() {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uColorGold: { value: new THREE.Color(0xffd88a) },
        uColorAsh: { value: new THREE.Color(0x8a8580) },
      },
      transparent: true,
      blending: THREE.AdditiveBlending,
      depthWrite: false,
  ...
  type: THREE.HalfFloatType, // <--- THIS IS CRASHING YOUR PHONE
    ...
    this.rtBlurB = new THREE.WebGLRenderTarget(w, h, opts);
    ...
    this.brightMat = new THREE.ShaderMaterial({
      ...
      uniforms: { uScene: { value: null }, uThreshold: { value: 0.35 } },
    });
    ...
    this.quadMesh = new THREE.Mesh(quadGeo, this.brightMat);
